resource "aws_instance" "cnxsmgmtatlassian" {
 ...
 network_interface {
 network_interface_id = aws_network_interface.cnxsmgmtatlassianinterfaceaz4.id
 device_index = 0
 }
 ...
 user_data = base64encode(templatefile("./userdata_bitbucket.sh", {}))
 tags = {
 Name = "cnxsmgmtatlassian"
 }
