...
function printdetail($name1, $name2, $sladetail) {
    ...
    $detailstr = null;
    $detailstr .= "<tr>\n<td>".$name1."</td><td>".$name2."</td><td>Downtime<div id=\"tzd\"></div></td><td>Uptime<div id=\"tzu\"></div></td><td>Duration</td><td>Message</td></tr>\n";
    foreach ($sladetail as $detail) {
        $dtdown = new DateTime($detail{"downtime"}, new DateTimeZone('UTC'));
	$dtdown->setTimezone(new DateTimeZone('US/Mountain'));
        $dtup = new DateTime($detail{"uptime"}, new DateTimeZone('UTC'));
	$dtup->setTimezone(new DateTimeZone('US/Mountain'));

        $detailstr .=  "<tr>\n<td></td><td></td><td id=\"nw\">";
        $detailstr .=  $dtdown->format('Y-m-d H:i:s')."<div id=\"tzd\">MDT</div>";
        $detailstr .=  "</td><td id=\"nw\">";
        $detailstr .=  $dtup->format('Y-m-d H:i:s')."<div id=\"tzu\">MDT</div>";
        $detailstr .=  "</td><td>";
