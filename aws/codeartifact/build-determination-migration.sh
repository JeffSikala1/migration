...
api_key="${bamboo_artifactory_admin_api_key_secret}"
...
if [[ "${BranchName:0:3}" = "${longLivedPrefix}" ]]; then
    ...
    mavenRepoExists=$(curl -s -X GET "$artifactoryRepositoryUrl?type=local&packageType=maven" | grep $BranchName | head -n1)
